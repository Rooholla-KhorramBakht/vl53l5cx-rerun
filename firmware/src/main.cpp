...
#define INT_PIN 21 //Connect VL53L5CX INT pin to pin 4 on your microcontroller
...
void loop()
{
  ...
  if (dataReady == true)
  {
    ...
    if (myImager.getRangingData(&measurementData)) //Read distance data into array
    {
      ...
      for(int i=0; i<64; i++)
        distance_packet.data.distance[i]=measurementData.distance_mm[i];

      Serial.write(distance_packet.buffer, 64*2+4);
    }
  }
